...
//    @PreAuthorize("hasRole('EMPLOYER')")
    public ResponseEntity<JobsDto> createJob(@RequestBody JobsDto jobDto, @PathVariable Long userId) {
...
//    @PreAuthorize("hasRole('EMPLOYER')")
    public ResponseEntity<JobsDto> updateJob(@PathVariable Long id, @RequestBody JobsDto jobDto, @PathVariable Long userId) {
        Users currentUser = userRepository.findById(userId)
                .orElseThrow(() -> new IllegalArgumentException("Invalid Id"));
//        Users currentUser = getCurrentUser(authentication);
...
//    @PreAuthorize("hasRole('EMPLOYER')")
    public ResponseEntity<Void> deleteJob(@PathVariable Long id, @PathVariable Long userId) {
        Users currentUser = userRepository.findById(userId)
                .orElseThrow(() -> new IllegalArgumentException("Invalid Id"));
//        Users currentUser = getCurrentUser(authentication);
